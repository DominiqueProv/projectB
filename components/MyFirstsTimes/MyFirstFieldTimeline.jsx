...
const MyFirstFieldTimeline = ({ item }) => {
  ...
  return (
    hasData && (
      <div className="flex gap-3 items-center bg-indigo-800 text-white p-1 px-3 justify-between rounded-md w-full sm:w-[400px] mt-4">
        <span className="flex-grow flex flex-col rounded-md">
          <span className="text-white font-semibold">
            {name || description}
          </span>
          {formatDateFirst(formDataFromDb[id].date)}
        </span>
        <button
